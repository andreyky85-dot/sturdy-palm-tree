...
Документ синхронизирован с текущим кодом. Автоматическая проверка env: `npm run check:env`. CI без секретов: `.github/workflows/ci.yml`. Прод на Vercel: `npm run build:vercel` (см. `vercel.json`).
